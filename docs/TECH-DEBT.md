...
## 8. One background block for every section

No section renders a CMS background yet. The hero keeps the hardcoded poster it has
always had, and only the video url comes from the CMS:

```
<section class="hero" style="background-image: url(img/hero_video.jpg);" id="hero">
...
<div class="hero__video js-hero-video" data-video-src="{{ heroBg.url }}"></div>
```

The data side is already there and shared: `background` (upload), `overlay`,
`overlayOpacity` and `paddingY` all come from `sectionStyleFields()` in
[fields/background.ts](../payload/src/fields/background.ts), so every section that opts
into the Style tab receives the same shape — nothing reads it.

Wanted: one reusable partial (or mixin) that takes the `background` upload and renders
the section's background itself — picking video or image off `mimeType`, and owning the
overlay and padding that ship with it. A section then declares its background instead of
building it, and the hardcoded posters go away.

Until it exists, sections keep their current hand-written background — deliberately, so
the branching is written once in the shared block rather than three times on the way
there.

- [ ] Write the shared background block and decide where it lives (`partials/payload/`)
- [ ] Move the hero onto it, then the sections that opt into the Style tab
- [ ] Fold `overlay` / `overlayOpacity` / `paddingY` into the same block
