---
description: Build conference, copy build output to archive year folder, and fix internal links. Use when the user wants to archive a conference (e.g., "archive aics", "archive the jsn build", "create year archive for rs"). The conference key is the argument.
argument-hint: <conf-key>
---

This skill archives a conference build into a year-specific folder with corrected internal links. The conference key is: $ARGUMENTS (e.g., `aics`, `jsn`, `rs`, `rdb`).

## Steps

### 1. Read the year from conference config

Read `src/conferences/$ARGUMENTS/conference-settings.js` and extract the year from `const eventYear = 'Y<YEAR>';`. Strip the `Y` prefix to get just the number (e.g., `Y2024` → `2024`).
...
In the `src/conferences/$ARGUMENTS/<YEAR>/` directory, find all `.html` files and replace internal links:

```bash
# Replace href="/" (root link) with href="/YEAR/"
find src/conferences/$ARGUMENTS/<YEAR>/ -name "*.html" -exec sed -i '' 's|href="/"|href="/<YEAR>/"|g' {} +

# Replace href="/something with href="/YEAR/something (but not href="// or already prefixed)
find src/conferences/$ARGUMENTS/<YEAR>/ -name "*.html" -exec sed -i '' 's|href="/\([a-z]\)|href="/<YEAR>/\1|g' {} +
```
