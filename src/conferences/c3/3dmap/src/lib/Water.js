      ...
      fragmentShader: (
        ...
        `
				...
				vec4 getNoise( vec2 uv ) {
					vec2 uv0 = ( uv / 103.0) + vec2(time / 17.0 / scale, time / 29.0 / scale);
					vec2 uv1 = ( uv / 107.0) - vec2( time / -19.0 / scale, time / 31.0 / scale);
					vec2 uv2 = (uv / vec2( 8907.0, 9803.0 )) + vec2( time / 101.0 / scale, time / 97.0 / scale);
					vec2 uv3 = (uv / vec2( 1091.0, 1027.0 )) - vec2( time / 109.0 / scale, time / -113.0 / scale);
				...
				void main() {
					...
					vec4 noise = getNoise( worldPosition.xz * size * scale );
					...
					vec3 reflectionSample = vec3( texture2D( mirrorSampler, (mirrorCoord.xy / mirrorCoord.w + distortion) ) ) / scale;
